...
exports.convetTool = void 0;
const sharp = require("sharp");
const path = require("path");
const fs = require("fs");
const convetTool = (inputDirectory, outputDirectory) => {
            ...
            sharp(inputFile)
                ...
                config.formats.forEach((format) => {
                    ...
                    config.sizes.forEach((size) => {
                        ...
                        const outputFile = path.join(outputFolder, `${baseFileName}-${size}.${format}`);
                        ...
                        sharp(inputFile)
                            ...
                            if (error) {
                                ...
                                // Log success message
                                console.log(`Successfully converted ${inputFile} to ${outputFile} in ${format} format`);
                            }
                        });
                    });
                    ...
                    config.scales.forEach((scale) => {
                        // Build the output file path
                        const outputFile = path.join(outputFolder, `${baseFileName}-${scale}x.${format}`);
                        // Use the sharp module to resize the image and save it to the output file path
                        sharp(inputFile)
