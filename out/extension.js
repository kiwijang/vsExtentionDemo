...
function activate(context) {
    ...
    let disposable2 = vscode.commands.registerCommand("hiworld.rc", async (uri) => {
        await vscode.commands.executeCommand("workbench.action.togglePanel");
        const folderPath = uri.fsPath;
        const arr = folderPath.split("\\");
        arr.pop();
        arr.push("output");
        (0, convert_1.convetTool)(folderPath, arr.join("\\"));
        vscode.window.showInformationMessage("產生成功 " + folderPath + "==>" + arr.join("\\"));
    });
